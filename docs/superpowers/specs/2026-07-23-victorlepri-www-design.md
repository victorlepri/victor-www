...
1. Grid: chunky square LED cells sized so about 64 rows fill the shorter viewport dimension; cell size = round(min(width, height) / 64), never below 7 css px. One grid for everything; background, rain, and name share the same pixel size. A split resolution variant was tested and rejected.
...
2. Leaf layers use the shared leafLayer function: diagonally tiled elongated dashes with dark midribs and per leaf random stretch. Layer one: tile (0.125, 0.25), thickness 1.8, density gate 0.32, weight 1.15. Layer two, the long lines: tile (0.045, 0.30), thickness 2.6, density gate 0.55, weight 1.25.
...
2. Displacement = (diamond normalized direction times 0.8 plus smoothed mouse velocity times 2.0) times ig; sweeping shoves the pattern along the motion, resting only quivers.
...
3. Projects overlay, two groups with pixel glyph headings:
   WORK: CΘSMOS LABS (theta replaces the O), role line IT Director, description: "The core team driving the Cosmos blockchain ecosystem. Cosmos powers the largest network of vertically integrated Layer 1 blockchains; 150+ chains securing more than $70B in assets for payments, tokenization, and digital currencies." Link: cosmos.network.
   BUILDS: STOOP NYC, tag "Find your next place, together.", description: "An iOS and browser app for deciding your next home, with your people. Paste a listing, score it solo, then the Reveal shows everyone's take. The best fit wins. Free on the App Store as Stoop NYC." Link: stoopnyc.app.
